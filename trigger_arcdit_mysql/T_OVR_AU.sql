...
or old.no_impegnato <> new.no_impegnato then
  ...
  if old.situazione <> 'annullato' then
	CALL P_MOVVEN_OVR('ovr', 'si', old.art_codice, old.tma_codice, old.situazione, old.progressivo,
  ...
  if new.situazione <> 'annullato' then
	CALL P_MOVVEN_OVR('ovr', 'no', new.art_codice, new.tma_codice, new.situazione, new.progressivo,
		...
		d_iva, d_mag, d_testata, d_importi_riga_origine, 'no');
  end if;
