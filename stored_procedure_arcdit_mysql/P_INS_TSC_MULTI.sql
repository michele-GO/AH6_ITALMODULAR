CREATE PROCEDURE P_INS_TSC_MULTI(IN p_codice varchar(10))
    MODIFIES SQL DATA
...
BEGIN
	...
	DECLARE cursore CURSOR FOR 
		...
		where arc.prd.prg_codice = 'GESTSC'
	...
	ciclo_inserimento: LOOP
   	...
   	set @query_controllo = CONCAT("select exists (select * from " , nome_db, ".tsc where codice = '", p_codice ,"') INTO @esiste;");
		PREPARE c from @query_controllo;
		EXECUTE c;
    		
    	IF @esiste <> 1 THEN
    		# se non esiste il codice viene creato
			set @query_inserimento = CONCAT("insert into " , nome_db, ".tsc (select * from " , schema(), ".tsc where codice = '", p_codice ,"');");
		ELSE
			# se esiste già viene aggiornato
			set @query_inserimento = CONCAT("replace into " , nome_db, ".tsc (select * from " , schema(), ".tsc where codice = '", p_codice ,"');");
		END IF;
