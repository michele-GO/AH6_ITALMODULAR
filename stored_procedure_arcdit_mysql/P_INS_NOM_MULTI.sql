CREATE PROCEDURE P_INS_NOM_MULTI(IN cod_nominativo VARCHAR(8))
    MODIFIES SQL DATA
    COMMENT 'inserimento / modifica di nominativi su archivi multiaziendali'
BEGIN
	...
	DECLARE cursore CURSOR FOR 
		...
		where arc.prd.prg_codice = 'GESNOM'
	...
	ciclo_inserimento: LOOP
		...
		set @query_controllo = CONCAT("select exists (select * from " , nome_db, ".nom where codice = '", cod_nominativo ,"') INTO @esiste;");
		PREPARE c from @query_controllo;
		EXECUTE c;
		DEALLOCATE PREPARE c;
    		
    	IF @esiste <> 1 THEN
			set @query_inserimento = CONCAT("insert into " , nome_db, ".nom (select * from " , schema(), ".nom where codice = '", cod_nominativo ,"');");
		ELSE
			set @query_inserimento = CONCAT("replace into " , nome_db, ".nom (select * from " , schema(), ".nom where codice = '", cod_nominativo ,"');");						
		END IF;	
