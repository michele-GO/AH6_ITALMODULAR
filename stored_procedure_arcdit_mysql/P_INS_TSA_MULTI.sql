CREATE PROCEDURE P_INS_TSA_MULTI(IN p_codice varchar(10))
    MODIFIES SQL DATA
...
BEGIN
	...
	DECLARE cursore CURSOR FOR 
		...
		where arc.prd.prg_codice = 'GESTSA'
	...
	ciclo_inserimento: LOOP
   	...
   	set @query_controllo = CONCAT("select exists (select * from " , nome_db, ".tsa where codice = '", p_codice ,"') INTO @esiste;");
		PREPARE c from @query_controllo;
		EXECUTE c;
    		
    	IF @esiste <> 1 THEN
    		# se non esiste il codice viene creato
			set @query_inserimento = CONCAT("insert into " , nome_db, ".tsa (select * from " , schema(), ".tsa where codice = '", p_codice ,"');");
		ELSE
			# se esiste già viene aggiornato
			set @query_inserimento = CONCAT("replace into " , nome_db, ".tsa (select * from " , schema(), ".tsa where codice = '", p_codice ,"');");
		END IF;	
